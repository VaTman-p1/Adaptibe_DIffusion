...
from losses import diffusion_loss, yaw_loss, goal_loss, path_loss, internal_consistency_loss, drift_consistency_loss
import matplotlib
matplotlib.use('Agg')
...
def visualize_all_channels(x0_gt, x0_pred, epoch, batch_idx, experiment, sample_idx=0):
    ...
    assert gt.shape[0] == 8, f"Ожидается 8 каналов, получено {gt.shape[0]}"
...
def run(cfg, base_save_dir="trajectory_diffusion/checkpoints"):
    ...
    exp_keys = ["lr", "channels", "goal_w", "yaw_w", "path_w","consist_w", "vconsist_w", "batch_size", "timesteps"]
    exp_name_parts = []
    for k in exp_keys:
        if k in cfg:
            val = cfg[k]
    ...
    for epoch in range(1, cfg["epochs"] + 1):
        model.train()
        train_metrics = {"diff": 0.0, "yaw": 0.0, "goal": 0.0, "path": 0.0, "intern_cons": 0.0, "drift_cons":0.0, "total": 0.0}
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch} [Train]"):
            ...
            pred = model(xt, t.float(), goal, context, force)
            x0_pred = reconstruct_x0(xt, pred, t, scheduler)

            diff_loss_value = diffusion_loss(pred, noise, mask)
            yaw_loss_value = yaw_loss(x0_pred, x0, mask)
            goal_loss_value = goal_loss(x0_pred, goal, last_point)

            drift_loss_value = drift_consistency_loss(x0_pred, mask)
            compensation_loss_value = internal_consistency_loss(x0_pred, mask, force)

            path_mask = (t.float() < cfg["path_t_thresh"]).float().view(-1, 1, 1)
            # print(path_mask)
            path_loss_raw = path_loss(x0_pred, mask)
            ...
            train_metrics["diff"] += diff_loss_value.item()
        ...
        with torch.no_grad():
            ...
            for idx, batch in enumerate(tqdm(val_loader, desc=f"Epoch {epoch} [Val]")):
                ...
                pred = model(xt, t.float() / cfg["timesteps"], goal, context, force)
